    ...
    def iter_episode(
        ...
        **kwargs,
        ...
        for idx in indices:
            ...
            head_stream = image_streams[0]
            hist_start = max(0, idx - history_len)
            hist_indices = list(range(hist_start, idx))
            if len(hist_indices) == 0:
                hist_indices = [0]
            if len(hist_indices) < history_len:
                hist_indices = [hist_indices[0]] * (history_len - len(hist_indices)) + hist_indices
            else:
                hist_indices = hist_indices[-history_len:]

            for hist_idx in hist_indices:
                hist_img = decode_image_from_bytes(head_stream[hist_idx])
                if image_aug:
                    hist_img = image_aug(hist_img)
                elif not isinstance(hist_img, torch.Tensor):
                    hist_img = torch.from_numpy(np.array(hist_img)).permute(2, 0, 1).float() / 255.0
                history_imgs.append(hist_img)

            while len(imgs) < self.num_views:
                imgs.append(torch.zeros_like(imgs[0]))

            image_input = torch.stack(imgs, dim=0)
            head_history = torch.stack(history_imgs, dim=0)
